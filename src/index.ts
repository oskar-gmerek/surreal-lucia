...
import { mapToLuciaDatabaseSession, mapToLuciaDatabaseUser } from "./helpers.ts";
...
export class SurrealDBLuciaAdapter implements Adapter {
	 ...
	 */
	constructor(db: Surreal, { sessionTableName, userTableName }: DatabaseTables) {
		this.db = db;
	...
	public async deleteUserSessions(userId: UserId): Promise<void> {
		await this.db.query(
			...
			{
				surrealSession: this.sessionTableName,
	...
	public async getSessionAndUser(
		...
		return [mapToLuciaDatabaseSession(result), mapToLuciaDatabaseUser(result.user)];
	}
	...
	public async getUserSessions(userId: UserId): Promise<DatabaseSession[]> {
		const result = (
			await this.db.query<[SurrealSession[]]>(
				...
				{
					sessionTable: this.sessionTableName,
					userTable: this.userTableName,
					userId: new RecordId(this.userTableName, userId),
				},
			)
	...
	public async deleteExpiredSessions(): Promise<void> {
		await this.db.query("DELETE type::table($sessionTable) WHERE expires_at < time::now()", {
			sessionTable: this.sessionTableName,
		});
	}
}
