...
import { Surreal, Table } from "@surrealdb/surrealdb";
import { surrealql } from "@surrealdb/surrealdb";
import { SurrealDBLuciaAdapter } from "../src/index.ts";
import type { SurrealSession, SurrealUser } from "../src/types.ts";
...
describe("SurrealDBLuciaAdapter", async () => {
	...
	test("should successfully deleteExpiredSessions()", async () => {
		const sessionTable = SURREALDB_TEST_SESSION_TABLE;
		...
		const expiredSessions = (await db.query<[SurrealSession[]]>(getExpiredSessionsQuery))[0];
